"""
Benchmark evaluation script for SlideBench (TCGA benchmark).

Evaluates trained WSI-VQA model on the benchmark test set and computes metrics.
    ...
    with torch.no_grad():
        for batch in tqdm(dataloader, desc="Evaluating"):
            ...
            for i in range(len(generated_ids)):
                # Get only the newly generated tokens (skip input prompt)
...
def parse_args():
    parser = argparse.ArgumentParser(description='Evaluate WSI-VQA on Benchmark')
    ...
    parser.add_argument('--llm_path', type=str, default='Qwen/Qwen3-4B-Instruct-2507',
                       help='Path to base Qwen LLM (same as training)')
...
def main():
    ...
    print("=" * 80)
    print("WSI-VQA Benchmark Evaluation")
    print("=" * 80)
